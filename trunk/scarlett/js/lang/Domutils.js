...
var Domutils = {
	...
	hasClassName: _Prototype.hasClassName,
	addClassName: _Prototype.addClassName,
	removeClassName: _Prototype.removeClassName,
	toggleClassName: _Prototype.toggleClassName,
	...
	bindElements: function(scope, element) {
		...
		for ( var child = element.firstChild; child; child = child.nextSibling ) {
			this.bindElements(scope, child);
		}
	},
	bindHandlers: function(scope, eventHelper) {
		for ( var prop in scope ) {
			if( !/.*_handler/.test(prop) ) {
				continue;
			}
			var targetName = prop.substring(0, prop.indexOf("_"));
			var element = scope[targetName];
			if ( element ) {
				;
			...
			eventHelper.attachListener(element, eventType, scope[prop], scope);
		}
	}
};
